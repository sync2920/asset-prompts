# 18. 夜明けの河川敷、マットキャミ＋透けシフォンスカートの横寄り歩行（参照画像ベース）

`ideas/README.md` の人物参照方針に準拠し、人物の顔、髪、肌、体型、身長感、プロポーションなどの身体的特徴は添付画像から忠実に保持する（髪を含む身体特徴は本文にハードコードしない）。`expression/01-sheer-skin-intimacy.md` の「透けは輪郭の影絵として描く」手法を使用する。朝焼けの光が体をどう読むかを見せ場にし、上＝マットキャミの肩・鎖骨への直接光、下＝透けシフォン越しの透過光の2態様を1主題に統一する。太陽そのものが地平線から顔を出してフレーム内に入る構図、右横寄りの斜め後ろ、振り返りなし。人物はベージュ〜クリーム〜ヌードのワントーン。

- **アスペクト比:** 9:16 縦長
- **見せ場:** 朝焼けの光の2態様 — 上の肩・鎖骨の直接暖光と、下のシフォン越しの透過光。右プロファイルがほぼ完全に読める（振り返りではない）
- **服装:** マットなベージュ/クリーム系キャミ（シルク/サテンのテカリなし）＋淡い花柄シフォンのミディ丈スカート（くるぶし少し上）＋ベージュ/ヌード系のフラットサンダル。小物・アクセサリーなし、ワントーン
- **背景:** 露の草むら・朝靄・遠景の鉄橋・空のベンチで河川敷の世界を埋めつつ人影ゼロ
- **文脈:** 誰もいない夜明けの散歩 × 朝焼けの逆光 × 透けシフォンの影絵 × ナチュラル＆上品カジュアル
...
### 既存朝案（ideas 197〜200）との差

- 197〜200 は「斜光が湯気・霧・カーテン・薄い布を照らす」媒質経由の光。本案は媒質を置かず、朝焼けの光が体をどう読むかを見せ場にする。上＝直接光、下＝透過光の2態様を1主題に統一。
- 角度は右横寄り斜め・振り返りなし。199 の「振り返りで横顔を見せる」とは明確に区別。

### 透けの安全設計（`expression/01` 準拠）

- シフォンの透けは「輪郭が影絵としてぼやっと」に限定。肌色・下着・裏地は透かさず、めくれ上がってウエスト下が見える状態を禁止。
- ミディ丈のため膝下は素足が出るが、これはスカート丈の結果であり意図的な露出設計ではない。

### ワントーン化

- 人物＝ベージュ＋クリーム＋ヌードの一系統、小花も地色に近いトーンオン톤で差し色なし。背景パレットの差し色記述も撤去。

### 足元の解剖学ネガ

- フラットサンダルで素足のつま先・甲が主役級に写るため、README の解剖学ネガの精神を足に適用：`five toes per foot, no extra toes, no malformed feet, no floating or doubled straps`。

### 観測失敗の対応（生成で確認済みの3点）

- スカートの風めくれ → めくれ禁止＋透けを「布が脚に触れる部分でのみ」に限定。
- 上下同色でワンピース化 → キャミをクリーム、スカートを一段濃い燕麦色に分離＋tuck でウエスト明確化。
- マキシ丈で靴が白い塊に → ミディ丈＋サンダルを具体で指定。
- いずれも独立 Avoid 行は新設せず、肯定文に織り込む（`expression/01` の「除外欄が逆に不安定にすることがある」を踏襲）。

### ハードコード精査

- 髪の長さ・型・色は参照依存、挙動だけ指定。プロファイル列挙は「何が読めるか」の指定で形・色の固定ではない。`fine peach-fuzz` は個人特徴でなくリアル肌のディテール要求。服装ブロック内の色・素材・柄・丈は「服装」指定であり身体特徴のハードコードではない。

### 参照

- `expression/01-sheer-skin-intimacy.md` — 透けを影絵シルエットで書く手法、線状境界（細ストラップ）の根拠、除外を肯定文に織り込む方針
- `ideas/356-riverside-dawn-back-three-quarter.md` — 本案のアイデアストック（6稿分の指摘反映履歴を含む）
